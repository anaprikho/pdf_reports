...
class PDFReportImages(PDFReport):
    ...
    def fill_with_images(self, list_of_images, no_per_site):
        ...
        no_per_column = 2  # desired number of columns on one page, may differ to the real number if there is not enough place due to calculated width

        if (no_per_site % no_per_column) == 0:  # calculate a width of a single image, a height will be then calculated automatically
        ...
        for j in range(0, rows_sum):
            ...
            if (j != rows_sum - 1) and y + 2.7 * width_of_image > (  # check if there is enough place for one more row on a page
...
pdf.fill_with_images(['images/bild1.png', 'images/bild2.png',
                      ...
                      'images/bild1.png', 'images/bild2.png'], no_per_site=8)  # the second argument is the number of images placed on a page
